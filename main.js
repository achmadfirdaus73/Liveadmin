...
function setupDatabaseListeners() {
    ...
    locationDataRef.on('value', (snapshot) => {
        // This will trigger updates to markers and stats
        filterUsers();
        updateStats();
    });
}
...
loginBtn.addEventListener('click', async () => {
    try {
        ...
        if (error.code === 'auth/unauthorized-domain') {
            showToast('Domain tidak terdaftar di Firebase. Silakan tambahkan domain ini di Firebase Console.');
        } else if (error.code === 'auth/popup-blocked') {
            showToast('Popup diblokir browser. Silakan izinkan popup untuk situs ini.');
...
function renderUserTable() {
    ...
    userTableBody.innerHTML = filteredUsers.map(user => `
        <tr class="user-card cursor-pointer" onclick="showUserModal('${user.id}')">
...
function showUserModal(userId) {
    ...
    modalUserLastUpdate.textContent = `Last Update: ${user.location?.timestamp ? 
        new Date(user.location.timestamp).toLocaleString() : 
        user.updatedAt ? 
        new Date(user.updatedAt).toLocaleString() : 
        'Never'
            ...
            const marker = L.marker([user.location.lat, user.location.lng])
                .bindPopup(`
                    <div>
                        ...
                        <button onclick="showUserModal('${user.id}')" class="mt-2 bg-blue-500 text-white px-2 py-1 rounded text-sm">
                            View Details
                        </button>
                    </div>
                `)
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        // Remove listeners to reduce network requests
        if (usersRef) {
            usersRef.off();
        }
        if (locationDataRef) {
